...
@NamedQueries({ @NamedQuery(name = "RegistrationPeriod.findAll", query = "SELECT r FROM RegistrationPeriod r"),
		...
		@NamedQuery(name = "RegistrationPeriod.findByIsLecturer", query = "SELECT r FROM RegistrationPeriod r WHERE r.isRegistrationLecturer = :isRegistrationLecturer AND r.isDeleted = 0"),
		@NamedQuery(name = "RegistrationPeriod.findByIsLecturerIsDeleteed", query = "SELECT r FROM RegistrationPeriod r WHERE r.isRegistrationLecturer = :isRegistrationLecturer AND r.isDeleted = 1"),
		@NamedQuery(name = "RegistrationPeriod.checkConditionsToCreateRegistrationPeriod", query = "SELECT r FROM RegistrationPeriod r WHERE r.semeter = :semeter AND r.schoolYear = :schoolYear AND r.closeDate <= :openDate AND r.isRegistrationLecturer = 1 AND r.isDeleted = 0"),
		@NamedQuery(name = "RegistrationPeriod.countBySchoolYearAndIsHead", query = "SELECT COUNT(r) FROM RegistrationPeriod r WHERE r.schoolYear = :schoolYear AND r.isRegistrationLecturer = :isRegistrationLecturer"),
...
public class RegistrationPeriod implements Serializable {
	...
	private byte isRegistrationLecturer;
	...
	public byte getIsRegistrationLecturer() {
		return this.isRegistrationLecturer;
	}

	public void setIsRegistrationLecturer(byte isRegistrationLecturer) {
		this.isRegistrationLecturer = isRegistrationLecturer;
	}
